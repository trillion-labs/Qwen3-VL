...
if [ -z "${OMPI_COMM_WORLD_SIZE-}" ] && [ -z "${PMI_SIZE-}" ]; then
  PROCS=${PROCS:-32}
  HOSTS=${HOSTS:-"sub6:8,sub7:8,sub8:8,sub9:8"}
  MASTER_ADDR=${MASTER_ADDR:-sub6}
  MASTER_PORT=${MASTER_PORT:-23500}
...
############################################
DEFAULT_MASTER_ADDR="sub6"     # main node
DEFAULT_MASTER_PORT="23500"    # choose an open fixed port for all ranks
